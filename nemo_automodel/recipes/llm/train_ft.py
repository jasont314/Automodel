...
def build_model_and_optimizer(
    ...
    if autopipeline is not None:
        ...
        if parallelize_fn is not None and get_world_size_safe() > 1:
            moe_mesh_for_parallelize = getattr(model_wrapper, "moe_mesh", None)
            parallelize_fn(
                model,
                world_mesh=model_wrapper.device_mesh,
                moe_mesh=moe_mesh_for_parallelize,
                pp_enabled=False,
                dp_axis_names=(
                ...
                tp_axis_name="tp",
                ep_axis_name=(
                    "ep" if moe_mesh_for_parallelize is not None and "ep" in moe_mesh_for_parallelize.mesh_dim_names else None
                ),
                ep_shard_axis_names=(
                    ("ep_shard",)
                    if moe_mesh_for_parallelize is not None and "ep_shard" in moe_mesh_for_parallelize.mesh_dim_names
                    else None
                ),
            )
            load_weights = True
...
def build_dataloader(
    ...
    with ScopedRNG(seed=seed, ranked=True):
        ...
        if pp_enabled:
            ...
            trust_remote_code = cfg_model.get("trust_remote_code", resolve_trust_remote_code(model_name))
            ...
            if is_nemotron_h:
                logging.info("PP enabled: skipping causal_mask_mapping precompute for NemotronH (unused in PP forward)")
            else:
